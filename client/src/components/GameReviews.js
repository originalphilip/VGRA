...
function GameReviews({ filter, onPlatformsFetched }) {
  ...
  const fetchPlatformsForGames = useCallback(
    (gameIds) => {
      ...
      .then((data) => {
        if (isMounted) {
                ...
                return filter.score.some((score) => {
                  if (score.includes("-")) {
                    const [minScore, maxScore] = score
                      ...
                      .map(parseFloat);
                    return (
                      review.AverageScore >= minScore &&
                      review.AverageScore <= maxScore
                    );
                  } else {
          ...
          if (previousGameIds.current !== gameIds) {
            ...
            previousGameIds.current = gameIds;
          }
        }
      })
    ...
    <div>
      {isLoading ? (
        ...
        reviews.map((review, index) => {
          const reviewURLs = review.ReviewURLs
            ? review.ReviewURLs.split(",")
            : [];
          const reviewSites = review.SourceWebsites
            ? review.SourceWebsites.split(",")
            : [];
          return (
